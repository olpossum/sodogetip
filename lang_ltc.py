...
link_withdraw = "https://www.reddit.com/message/compose?to=" + config.bot_name + "&subject=%2Bwithdraw&message=%2Bwithdraw%20AMOUNT%20ltc%20to%20ADDRESS"
...
message_account_details = "\n\nHere are your account details /u/{{ username }}!" \
                            "\n\n^very ^info | &nbsp;" \
                            "\n---|---" \
                            "\n^Your ^balance | ^{{ spendable_balance }} ^ltc ^(${{ spendable_value_usd }})" \
                            "\n^Your ^pending ^balance | ^{{ pending_balance }} ^ltc ^(${{ pending_value_usd }})" \
                            "\n^Tips ^to ^unregistered ^users | ^{{ pending_tips }} ^ltc ^(${{ pending_tips_value_usd}})" \
                            "\n^Deposit ^address | ^{{ address }}" \
                            "\n^Withdraw | ^[+withdraw](" + link_withdraw + ")" \
                            "\n\n__ATTENTION:__ This bot is \"on chain\" so for every tip you make, you'll pay a small fee (typically 1 ltc)!"
...
message_tip = ("__^[wow ^so ^verify]__: ^/u/{{ sender }} ^-> ^/u/{{ receiver }} ^__Ð{{ amount }}__ ^__ltc__ ^__(${{ value_usd }})__ ^[[help]](" + link_help + ")  ^[[transaction]](https://chain.so/tx/ltc/{{ txid }})")
message_withdraw = ("__^[wow ^so ^verify]__: ^/u/{{ username }} ^-> ^{{ receiver_address }} ^__Ð{{ amount }}__ ^__ltc__ ^__(${{ value_usd }})__ ^[[help]](" + link_help + ")")
message_withdraw_to_self = ("__^[such ^error]__: ^You ^cannot ^withdraw ^to ^your ^own ^bot ^address")
message_footer = ("\n\n*****" \
                 "\n\nNew to litecoin or " + config.bot_name + "? Ask the community any questions on /r/litecoin!" \
                 "\n\n^quick ^commands |&nbsp;" \
                 "\n---|---" \
                 ...
                 "\n\n__PROTIP:__ An example tip would be: +/u/" + config.bot_name + " 100 ltc")

message_help = ("Welcome to litecoin and " + config.bot_name + "!\n\nThe community is extremely important to us, and we\'re always happy to see new faces and help people like you get excited about the most fun and social coin out there :)\n\nTo learn more about litecoin and experience our friendly community, check out the /r/litecoin subreddit. Don't worry, we don't bite (unlike real-life shibes).\n\nUnlike other tipping services," + config.bot_name + " is built on an \"on chain\" platform. This means every tip is verified by the secure litecoin network and publically viewable on its database of transactions (called a blockchain) for a small fee (typically 0.001 ltc).\n\nThis is your wallet address: {{ address }}\n\nHow to use " + config.bot_name + ":\n\nSend someone a litecoin tip by commenting or replying: +/u/" + config.bot_name + " AMOUNT ltc. Replace AMOUNT with an amount of litecoins (ex. 100 to tip 100 ltc).\n\n[+history](" + link_history + ") - view your transaction history (includes tips and withdraws)\n\n[+info](" + link_info + ") or [+balance](" + link_balance + ") - view your litecoin balance on /u/" + config.bot_name + "\n\n[+withdraw](" + link_withdraw + ") - withdraw litecoins from your /u/" + config.bot_name + " balance into a separate litecoin address")
...
message_recipient_need_register_title = ("Someone sent you a litecoin tip of Ð{{ amount }}, and you need to register to receive it!")
message_recipient_need_register_message =  ("Hello /u/{{ username }}! You need to register an account before you can receive __{{ sender }}\'s__ litecoin tip of __Ð{{ amount }} ltc (${{ value_usd }})__." \
                        "\n\nTo register an account:" \
                        "\n\n1. [Click here](" + link_register + ") to send a pre-filled +register message" \
                        "\n\n2. Click the \"Send\" button" \
                        "\n\n3. Receive the successful register message" \
...
message_gold_no_enough_ltc = "Sorry it seems your confirmed balance is too low"
